...
## D. Representative work
* AutoFDO: automatic feedback-directed optimization for warehouse-scale applications
* BOLT: A Practical Binary Optimizer for Data Centers and Beyond
* AsmDB: understanding and mitigating <font color="#2DC26B">front-end stalls</font> in warehouse-scale computers
* I-SPY: Context-Driven <font color="#2DC26B">Conditional Instruction Prefetching</font> with Coalescing
* Ripple: profile-guided <font color="#2DC26B">instruction cache replacemen</font>t for data center applications
* Twig: Profile-Guided <font color="#2DC26B">BTB Prefetching</font> for Data Center Applications
...
### A. 计算average loop execution time & average loop trip count
...
得到average loop execution time之后还是得不到IC_latency, MC_latency
为此，这篇文章做了delinquent loads (loads that cause frequent LLC misses)的latency distribution
1. Capture delinquent load PCs that induce frequent Last Level Cache (LLC) misses
2. Capture LBR samples at the default frequency of once per millisecond while executing the application
3. Search all LBR samples that contain the delinquent load PC (load PC大于BBL start PC 小于teminating branch PC)
4. for all LBR samples that contain at least two instances of the BBL containing the delinquent load, measure the loop execution time by subtracting the cycle counts of the two subsequent branches
5. analyze the latency distribution of the loop's execution time to predict the latency in the case that the load is served from the L1 or L2 cache
...
## Finding the Optimal Prefetch Injection Site
